...
/* Value scale, calibrated against the known market values in the dataset
   (n=3,769: min 8k, median 79k, max 150k) -- not against global transfer
   headlines, since these are lower-division players. */
const VALUE_FLOOR = 10000.0, VALUE_CEIL = 500000.0;
const VALUE_MEDIAN = 79000.0, VALUE_SPREAD = 3.15, VALUE_BASE_REF = 0.488432;
...
const FEE_REF = 10000.0, FEE_EXP = 0.35;
...
/* Log-scale: a reference-quality player at peak age in a tier-2 league with a
   ...
   multiplicatively. The old engine was a linear ramp hard-clamped to 9k-160k
   with a random jitter term that added +-10% of pure noise to a number users
   read as an estimate; both are gone. */
...
function acquirability(p) {
  ...
  const vEff = Math.max(p.displayMarketValue || 79000.0, FEE_REF);
